...
//    }
    ...
    if (cell[index].type == FLUID || cell[get_grid_index(grid_pos + ivec3(-1, 0, 0))].type == FLUID) {
        // check solid
        if (grid_pos.x == 0 || grid_pos.x == grid_dim.x - 1 || cell[index].nType==1) {
            cell[index].vel.x = 0;
        } else {
    ...
    if (cell[index].type == FLUID || cell[get_grid_index(grid_pos + ivec3(0, -1, 0))].type == FLUID) {
        // check solid
        if (grid_pos.y == 0 || grid_pos.y == grid_dim.y - 1 || cell[index].nType==1) {
            cell[index].vel.y = 0;
        } else {
    ...
    if (cell[index].type == FLUID || cell[get_grid_index(grid_pos + ivec3(0, 0, -1))].type == FLUID) {
        // check solid
        if (grid_pos.z == 0 || grid_pos.z == grid_dim.z - 1 || cell[index].nType==1) {
            cell[index].vel.z = 0;
        } else {
